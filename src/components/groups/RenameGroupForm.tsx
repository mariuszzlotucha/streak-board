...
export default function RenameGroupForm({ name: currentName }: RenameGroupFormProps) {
  ...
  function validate() {
    ...
    setError(next);
    if (next) document.getElementById("group-name")?.focus();
    return !next;
  }
  ...
  return (
    <form method="POST" action="/api/groups/rename" className="space-y-4" onSubmit={handleSubmit} noValidate>
      <FormField
        id="group-name"
        name="name"
        label="Group name"
