...
- **Rule**: Always list `/10x-research <change-id>` as a regular, numbered step in the copy-pasteable command list for a new slice, right after `/10x-new` and before `/10x-plan`, in the order `/10x-new`, `/10x-research`, `/10x-plan`, `/10x-plan-review`, git commit and push of the artifacts on `master`, then the `/10x-implement` phases. Do not label it optional; the user decides whether to run it.
- **Applies to**: frame, research, plan, plan-review
